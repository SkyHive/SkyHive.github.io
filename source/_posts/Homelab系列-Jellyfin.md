...
![首页 Banner](https://blogpic.skyhive.tech/pic/jellyfin/banner.png)
![电影墙](https://blogpic.skyhive.tech/pic/jellyfin/movie_wall.png)
![美化效果](https://blogpic.skyhive.tech/pic/jellyfin/detail_page.png)

#### 媒体库配置

按照我个人的习惯，将媒体库分成了四个部分 —— `电影`、`电视剧`、`纪录片`，以及那个**只可意会不可言传**的 `九公斤`。分别对应 `Movie`、`Series`、`Documentary` 以及 `Villa` 四个挂载进来的目录。

> `九公斤` 到底是什么？咳咳，这是一个关于**成人向**的深奥话题，为了保持本文的纯洁性，咱们留到后续的某篇文章中再单独探讨。
...
> 想要拥有一面完美的“海报墙”，光靠插件是不够的。**文件的命名规范**、**目录结构**以及 **刮削的调教** 都是一门玄学。
>
> 这部分内容实在太过于庞大（且充满了踩坑血泪史），所以我决定将其剥离出来，作为 `HomeLab` 系列的独立篇章。
>
> 今天，为了让大家先跑起来，我们只进行最基础的*能用就行*的配置。
